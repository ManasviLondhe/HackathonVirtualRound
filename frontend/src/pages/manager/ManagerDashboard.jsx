...
import { pendingApprovals, approvalHistory, teamExpenses } from "../../services/api";
import DashboardLayout from "../../components/DashboardLayout";
import { Clock, CheckCircle, XCircle, Users, FileText } from "lucide-react";

export default function ManagerDashboard() {
  const [stats, setStats] = useState({ pending: 0, approved: 0, rejected: 0, team: 0 });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all([pendingApprovals(), approvalHistory(), teamExpenses()])
      .then(([pendRes, histRes, teamRes]) => {
  ...
  const cards = [
    ...
    { label: "Team Expenses", value: stats.team, icon: Users, color: "bg-indigo-500" },
  ];
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">Manage your team's expenses</p>
      </div>
